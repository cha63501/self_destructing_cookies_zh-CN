/* Copyright (c) 2012, 2013 Ove Sörensen <sdc@elektro-eel.org>
...
function setupAsyncPhase() {
  ...
  refreshCookieRefs();
  observer.add("cookie-changed", handleCookieChanged);
  if (localStorage) observer.add("dom-storage2-changed", handleDomStorageChanged);
  observer.add("user-interaction-active", function(subject, data){ handleIdle("user-interaction-active", subject, data); });
  observer.add("user-interaction-inactive", function(subject, data){ handleIdle("user-interaction-inactive", subject, data); });
}
...
function handleIdle(topic, subject, data) {
...
function handleCookieChanged(subject, data) {
  ...
  switch (data) {
    ...
    case "deleted":
      ...
      break;
    case "batch-deleted":
      var cookies = subject.QueryInterface(Ci.nsIArray);
...
function handleDomStorageChanged(subject, data) {
  // TODO we should ignore SessionStorage, but how?
  var event = subject;
  try {
    event = subject.QueryInterface(Ci.nsIDOMStorageEvent);
  } catch(e) {
    // ignore
  }
  ...
  if (event.oldValue != null && event.newValue == null) {
    ...
    // entry added
    domStorageHelper.track(event.url);
  }
}
